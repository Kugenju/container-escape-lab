...
- 最新 CVE 检索（runc 2025 系列）已优先完成：`CVE-2025-31133` 已复现并完成 Docker+iSulad 同步验证；`CVE-2025-52565` 与 `CVE-2025-52881` 已补录官方 Buildx 公布模板并完成 Docker+iSulad 复测，当前分别阻断于远端模板镜像/前端镜像可达性。
...
- Buildx 触发链补录（`2026-03-11`）：
  ...
  - `CVE-2025-52565` 进入 named-context 解析阶段，阻断于 `cyphar/procfs-trap-in-docker-buildx` registry 可达性。
  - `CVE-2025-52881` 进入 Dockerfile frontend 解析阶段，阻断于 `docker/dockerfile:1.20.0-rc.1` registry 可达性。
