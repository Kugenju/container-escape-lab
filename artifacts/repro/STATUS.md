...
- 最新 CVE 检索（runc 2025 系列）已优先完成：`CVE-2025-31133` 已复现并完成 Docker+iSulad 同步验证；`CVE-2025-52565` 已在“本地 registry 重放模板”路径进入 `PROBE_EXECUTED`；`CVE-2025-52881` 在镜像可达性补齐后阻断前移为 `dockerfile_add_from_not_supported`。
- 联网新增 `CVE-2025-23266` PoC 来源（`jpts/cve-2025-23266-poc`）并完成 Docker+iSulad 实测；Docker 侧已注册 `nvidia` runtime 并进入 hook 阶段，但阻断于宿主缺失 `libnvidia-ml.so.1`，iSulad 侧仍不支持 `runtime nvidia`。
...
| CVE-2025-52565 | imported Buildx advisory procfs template (`COPY --from=vuln-container /proc/sys/kernel/core_pattern /x`): official remote template path blocked on `docker.io/cyphar/...` metadata pull (`BLOCKED_STAGE=exploit_context_image_unreachable`), then replayed via local registry (`172.17.0.1:5000`) and reached `PROBE_EXECUTED` | synchronized isula probe confirms no buildx/named-context equivalent entry (`BLOCKED_STAGE=isula_buildx_named_context_unsupported`) | `artifacts/repro/docker/CVE-2025-52565/buildx-template-20260311-attempt1/`, `artifacts/repro/docker/CVE-2025-52565/local-registry-20260311-attempt2/`, `artifacts/repro/isula/CVE-2025-52565/isula2.1.6-sync-20260311-attempt1/` |
| CVE-2025-52881 | imported Buildx advisory sysfs template (`# syntax=docker/dockerfile:1.20.0-rc.1` + `ADD --from=vuln-container ...`): official path first blocked at frontend image pull (`BLOCKED_STAGE=dockerfile_frontend_image_unreachable`); after switching to mirror frontend and local context replay, blocker moved to parser capability mismatch (`BLOCKED_STAGE=dockerfile_add_from_not_supported`) | synchronized isula probe confirms no buildx/named-context equivalent entry (`BLOCKED_STAGE=isula_buildx_named_context_unsupported`) | `artifacts/repro/docker/CVE-2025-52881/buildx-template-20260311-attempt1/`, `artifacts/repro/docker/CVE-2025-52881/local-registry-frontend120-20260311-attempt3/`, `artifacts/repro/docker/CVE-2025-52881/local-registry-frontend-masterlabs-20260311-attempt6/`, `artifacts/repro/isula/CVE-2025-52881/isula2.1.6-sync-20260311-attempt1/` |
| CVE-2025-23266 | imported upstream PoC (`jpts/cve-2025-23266-poc`), after enabling `nvidia-container-toolkit 1.17.7` build succeeded but runtime hook failed on missing `libnvidia-ml.so.1` (`BLOCKED_STAGE=nvidia_driver_library_unavailable`) | synchronized rerun under iSulad profile remained blocked because daemon rejects `runtime nvidia` (`BLOCKED_STAGE=isula_nvidia_runtime_unavailable`) | `artifacts/repro/docker/CVE-2025-23266/docker20-runc1.1.5-20260311-attempt1/`, `artifacts/repro/docker/CVE-2025-23266/docker20-runc1.1.5-nct1.17.7-20260311-attempt6/`, `artifacts/repro/isula/CVE-2025-23266/isula2.1.6-runc1.1.5-20260311-attempt1/`, `artifacts/repro/isula/CVE-2025-23266/isula2.1.6-runc1.1.5-nct1.17.7-20260311-attempt2/` |
...
- Buildx 触发链补录（`2026-03-11`）：
  - installed local Buildx plugin `v0.31.0`; on Docker `20.10.24` default driver first hit `moby.buildkit.frontend.contexts` unsupported.
  - switched to `docker-container` builder (`buildkit v0.27.1`) to align with official templates.
  - `CVE-2025-52565` 官方远端模板阻断于 `cyphar/procfs-trap-in-docker-buildx` 可达性；通过本地 registry 重放后可 `PROBE_EXECUTED`。
  - `CVE-2025-52881` 官方远端模板先阻断于 frontend 镜像可达性；镜像可达后阻断前移为 `BLOCKED_STAGE=dockerfile_add_from_not_supported`（可用 frontend 未支持 `ADD --from`）。
