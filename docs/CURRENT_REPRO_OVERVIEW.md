...
4. iSulad 映射复测：对关键 CVE 做等价触发并记录语义差异。
...
| CVE-2019-5736 | `BLOCKED_STAGE=runc_exe_handle_not_observed`（含 `runc 1.0.0-rc5` 多轮复测） |
...
- `CVE-2019-5736`：`runc 1.0.0-rc5` 多变体复测后仍未形成宿主机 proof，结论收敛到句柄未观测。
...
2. CVE-2019-5736 在脆弱版本下仍未命中宿主机 proof，后续可继续做更细粒度句柄竞争观测。
