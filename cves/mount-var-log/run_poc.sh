...
"$HELPER" "$NAMESPACE" "$POD_NAME" 'id; (command -v lsh >/dev/null 2>&1 && lsh / | head || echo "lsh not found"); (command -v cath >/dev/null 2>&1 && cath /etc/hostname || echo "cath not found")'
